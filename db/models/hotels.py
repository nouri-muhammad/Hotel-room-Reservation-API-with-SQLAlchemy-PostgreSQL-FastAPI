...
from sqlalchemy.types import String, Integer, Date, DateTime, Boolean, Enum, Float, ARRAY
...
class State(Base):
    ...
    def __init__(self, state_name, country_id):
...
class Hotel(Timestamp, Base):
    ...
    hotel_name: Mapped[str] = mapped_column("hotel_name", String(100), nullable=False)
    ...
    manager: Mapped[str] = mapped_column("manager", String(150), nullable=True)
    city: Mapped["City"] = relationship(back_populates= "hotels")
    rooms: Mapped[Set["Rooms"]] = relationship(back_populates="hotel")

    def __init__(self, city_id, hotel_name, web_address, stars, address, description, manager):
...
class RoomStatus(Timestamp, Base):
    ...
    booked_date: Mapped[List[date]] = mapped_column("booked_date", ARRAY(Date()), nullable=True)
    rooms: Mapped["Rooms"] = relationship(back_populates="status")

    def __init__(self, room_id, booked_date):
...
class Floor(Timestamp, Base):
    ...
    rooms: Mapped[List["Rooms"]] = relationship(back_populates="floor")
...
class RoomFeature(Timestamp, Base):
    ...
    id: Mapped[int] = mapped_column("id", Integer(), autoincrement=True, primary_key=True)
    features: Mapped[str] = mapped_column("features", String(400), nullable=True)
...
class BedTypeName(str, enum.Enum):
    standard = "standard"
    queen = "queen"
    king = "king"
...
class BedType(Timestamp, Base):
    ...
    def __init__(self, bed_type_name):
...
class Rooms(Timestamp, Base):
    ...
    room_number: Mapped[str] = mapped_column("room_number", String(15), nullable=False)
    capacity: Mapped[int] = mapped_column("capacity", Integer(), nullable=False)
    bed_num: Mapped[int] = mapped_column("num_beds", Integer(), nullable=False)
    ...
    def __init__(self, hotel_id, floor_id, room_class_id, room_number, capacity, bed_num, price):
...
class UserInfo(Timestamp, Base):
    ...
    def __init__(self, first_name, last_name, user_id, social_security_num, phone_num, email):
...
class Address(Timestamp, Base):
    ...
    address_line2: Mapped[str] = mapped_column("address_line2", String(200), nullable=True)
    ...
    def __init__(self, country_id, state_id, city_id, user_id, address_line1, address_line2, street, unit_num):
        self.country_id = country_id
        self.state_id = state_id
        self.city_id = city_id
        self.user_id = user_id
